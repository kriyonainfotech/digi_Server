...
const updateCssToJson = async (req, res) => {
  try {
    const { id, cssText } = req.body; // Extract ID and CSS text from the request body
    console.log(req.body);
    ...
    let updatedImageUrl = existingFrame.imageUrl; // Retain the existing image URL if no new image is provided

    // Check if a new image file is uploaded
    if (req.file) {
      updatedImageUrl = req.file.path; // Update with the new file path
    }
    ...
    existingFrame.jsonData = jsonOutput;
    existingFrame.imageUrl = updatedImageUrl;

    await existingFrame.save();

    res.status(200).json({
      success: true,
      message: 'CSS JSON and image updated successfully.',
      data: existingFrame,
    });
  } catch (error) {
    console.error('Error:', error);
...
module.exports = {
  convertCssToJson,getFrameJson,deleteframe,namegenrate,getTotalFrameCount,updateCssToJson
};